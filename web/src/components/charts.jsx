...
export function AreaChart({ data, xKey, yKey, height = 220, color = '#3987e5', format = v => v }) {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {ticks.map((t, i) => (
          <g key={i}>
            <line x1={pad.left} x2={W - pad.right} y1={t.y} y2={t.y} stroke="#2c2c34" strokeWidth="1" />
            <text x={pad.left - 8} y={t.y + 4} textAnchor="end" fontSize="11" fill="#898791">
              {t.label}
            </text>
          </g>
        ))}
        {points.map((p, i) =>
          i % Math.ceil(points.length / 7) === 0 || i === points.length - 1 ? (
            <text key={i} x={p.x} y={H - 8} textAnchor="middle" fontSize="11" fill="#898791">
        ...
        {hover && (
          <>
            <line x1={hover.x} x2={hover.x} y1={pad.top} y2={H - pad.bottom} stroke="rgba(255,255,255,0.25)" strokeDasharray="3 3" />
            <circle cx={hover.x} cy={hover.y} r="4.5" fill={color} stroke="#141826" strokeWidth="2" />
          </>
        )}
      </svg>
      {hover && (
...
export function BarList({ items, nameKey, valueKey, format = v => v, color = '#3987e5' }) {
