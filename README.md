...
- **通知公告** — 多条、分级别(通知 / 提醒 / 重要)、可置顶、可存草稿。控制台顶部只挂**最重要的那一条**
  (置顶优先,否则最新),一次铺开全部等于什么都没说;侧边栏带未读角标,打开公告页即全部已读。
  修改已发布的公告不会重置发布时间 —— 改个错别字不该让所有人重新收到提醒。
  未登录也能读(落地页要在没登录时就显示「今晚维护」),草稿只有管理员看得到
